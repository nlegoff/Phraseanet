...
function giveMeBases(Application $app, $usr = null)
{
    ...
    foreach ($app['phraseanet.appbox']->get_databoxes() as $databox) {
        ...
        foreach ($databox->get_collections() as $collection) {
            ...
            } elseif (! $cguSpec && $collInscript) {
                //ne va pas.. si l'inscriptio na la coll est explicitement non autorise, je refuse'
                $inscriptions[$sbas_id]['Colls'][$collection->get_coll_id()] = $collection->get_label($app['locale.I18n']);
            } elseif ($cguSpec) {
...
function giveMeBaseUsr(Application $app, $usr, $lng)
{
    ...
    foreach ($inscriptions as $sbasId => $baseInsc) {
        //je presente la base
        if (($baseInsc['CollsRegistered'] || $baseInsc['CollsRefuse'] || $baseInsc['CollsWait'] || $baseInsc['CollsIntime'] || $baseInsc['CollsOuttime'] || $baseInsc['CollsNonactif'] || $baseInsc['CollsCGU'] || $baseInsc['Colls'])) {
            //&& $baseInsc['inscript'])
        ...
        if (($baseInsc['CollsCGU'] || $baseInsc['Colls']) && $baseInsc['inscript']) {
            // il y a des coll ou s'inscrire !
            $noDemand = false;

            if ($baseInsc['Colls']) {
                //des coll ou on peut s'inscrire sans cgu specifiques

...
//      , 'Exclude Status', false);
        ...
        $this->addOption('base_id', 'b', InputOption::VALUE_REQUIRED | InputOption::VALUE_IS_ARRAY, 'Restrict on base_ids', array());

        $this->addOption('sbas_id', 's', InputOption::VALUE_REQUIRED | InputOption::VALUE_IS_ARRAY, 'Restrict on sbas_ids', array());
    ...
    protected function processRecords(\record_adapter $record, $outfile, $caption)
    {
        ...
        touch(
            $dest_file->getPathname(), $record->get_creation_date()->format('U'), $record->get_modification_date()->format('U')
        );
